...
RSpec.describe Post, type: :model do
  ...
  context 'Scopes' do
    it 'default_scope shows posts in descending order by created_at' do
      first_post = create(:post)
      second_post = create(:post)
      expect(Post.all).to eq [second_post, first_post]
    end

    it 'by_category scope gets post by particular category' do
    ...
    it 'search bar finds matching post' do
  ...
  context 'Validations' do
    ...
    it 'is not valid if title is longer than 255 characters' do
      @post.title = 'a' * 260
      expect(@post).not_to be_valid
    end
    ...
    it 'is not valid with no content' do
     ...
     it 'is not valid with content less than 20 characters' do
       @post.content = 'a * 10'
       expect(@post).not_to be_valid
     end

     it 'is not valid if content is greater than 1000 chracters' do
       @post.content = 'a' * 1001
       expect(@post).not_to be_valid
     end
  end
